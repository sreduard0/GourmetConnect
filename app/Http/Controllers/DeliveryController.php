...
class DeliveryController extends Controller
{
    ...
    public function delivery_information_modal($id)
    {
        ...
        switch ($requestData->status) {
            case 1:
                $status = 'NOVO PEDIDO';
                if (Auth::user()->hasPermissionTo('sts_delivery')) {
    ...
    public function delivery_table(Request $request)
    {
        ...
        foreach ($deliverys as $delivery) {
            ...
            if (Auth::user()->hasPermissionTo('view_delivery')) {
                $buttons .= '<button onclick="return delivery_client_view_modal(\'' . Tools::hash($delivery->id, 'encrypt') . '\')" class="btn btn-sm btn-default" ><i class="fa-solid fa-eye"></i></button> ';
            }
            if (Auth::user()->hasPermissionTo('delete_delivery')) {
            ...
            switch ($delivery->status) {
                case 1:
                    $status = 'Novo';
                    if (Auth::user()->hasPermissionTo('sts_delivery')) {
                        $buttons .= '<button onclick="return print_request(\'' . Tools::hash($delivery->id, 'encrypt') . '\')" class="btn btn-sm btn-info"><i class="fa-solid fa-print"></i></button>';
                    }
                    break;
                case 2:
                    $status = 'Em andamento';
                    if (Auth::user()->hasPermissionTo('sts_delivery')) {
                        $buttons .= '<button onclick="return out_for_delivery(\'' . Tools::hash($delivery->id, 'encrypt') . '\',\'' . Tools::hash(3, 'encrypt') . '\')" class="btn btn-sm btn-primary"><i class="fa-solid fa-moped"></i></button>';
                    }
                ...
                case 3:
                    $status = 'Saiu para entrega';
                    if (Auth::user()->hasPermissionTo('sts_delivery')) {
                        $buttons .= '<button onclick="return finalize_delivery(\'' . Tools::hash($delivery->id, 'encrypt') . '\',\'' . Tools::hash(4, 'encrypt') . '\')" class="btn btn-sm btn-success"><i class="fa-solid fa-check"></i></button>';
                    }
                    break;
